...
export const SkillCard: React.FC<SkillCardProps> = ({ skill }) => {
  ...
  const getProficiency = (level: number): string => {
    if (!t.proficiencyLevels) return '';
    if (level >= 9) return t.proficiencyLevels.expert;
    if (level >= 7) return t.proficiencyLevels.advanced;
    if (level >= 5) return t.proficiencyLevels.proficient;
    if (level >= 3) return t.proficiencyLevels.intermediate;
    return t.proficiencyLevels.beginner;
  };
